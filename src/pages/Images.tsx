...
import { Upload, LockKeyhole, LogOut, Image as ImageIcon, Archive } from 'lucide-react';
    ...
    <>
      ...
      {!logged ? (
        <div className="login-panel">
          ...
          <h2>你的图片，妥善安放</h2>
          <p>图片链接可以公开分享。图片列表与上传仅对站主开放。</p>
          <form onSubmit={login}>
            <label htmlFor="admin-token">管理员凭据</label>
            <input
              id="admin-token"
              type="password"
              autoComplete="off"
              ...
              placeholder="输入站点管理员密钥"
            />
            <button className="button primary" disabled={busy}>
              {busy ? '正在连接…' : '进入图片空间'}
            </button>
          </form>
          <small>凭据只保留在当前页面内存，刷新或退出后清除。</small>
        </div>
      ) : (
        <>
