...
response = await fetch(`${base}/api/images`, { method: 'POST', headers, body: '<svg>unsafe</svg>' });
assert.equal(response.status, 415);
response = await fetch(`${base}/api/images`, { method: 'POST', headers: { ...headers, Origin: 'https://untrusted.test' }, body: 'test' });
...
response = await fetch(`${base}/api/images`, { method: 'POST', headers: { ...headers, 'Content-Type': 'image/png', 'X-Filename': encodeURIComponent('本地测试.png') }, body: png });
...
assert.equal((await fetch(`${base}/api/images/${image.id}`, { method: 'DELETE', headers })).status, 200);
response = await fetch(`${base}/api/images`, { headers });
assert.ok(!(await response.json()).images.some(item => item.id === image.id));
assert.equal((await fetch(`${base}/api/images/${image.id}`, { method: 'POST', headers })).status, 200);
response = await fetch(`${base}/api/images`, { headers });
assert.ok((await response.json()).images.some(item => item.id === image.id));
...
console.log('API integration: auth, origin, type validation, R2 upload/read/HEAD/ETag, D1 list/archive/restore and cursor checks passed.');
