...
文件与工具优先：首页展示自己的文件，也可直接打开开发工具、流程图和画布，内置可操作的 JSON 格式化/压缩区。Base64、URL、时间戳、哈希支持直接链接到对应工具。文章作为辅助内容保留，管理后台增加已加载文章的关键词与发布状态筛选。

后端统一到 `server/api/`：`app.ts` 管理鉴权和错误边界，文章、图片、云草稿按业务分组；`functions/api/` 保留 Pages 路由入口。原有文章表、文章地址、图片地址和本地草稿格式均兼容。多文档新增 `0004_tool_documents.sql`；连接文件库前须确认目标 D1 已执行该迁移。历史说明不代表当前远程迁移状态。
...
如果已安装 Chrome，可设置环境变量 `PLAYWRIGHT_CHANNEL=chrome`，使用现有浏览器代替下载 Chromium。先执行 `npm run build`，再运行 `npm run test:browser`；端口未占用时测试会自动启动隔离的 Pages 服务，本地已有服务时复用它。浏览器测试只连接固定的本地端口，覆盖文本转换、流程图导出、画布绘制/撤销、文章发布/草稿隔离、图片上传/归档恢复、慢请求云同步、键盘跳转和六种屏幕宽度，同时运行图片、文章、文件三组 API 集成测试。
...
1. 首页“我的文件”输入现有 `ADMIN_TOKEN`，点击“连接文件库”。密钥仅保留在页面内存，刷新或换设备后重新连接。
2. 新建流程图或画布，填写名称；编辑会自动保存。首页支持搜索、类型筛选、重命名、独立复制、归档与恢复，按最近打开排序。
3. 在其他设备连接同一个站点，打开同名文件继续编辑。多人或多窗口同时修改同一文件时，旧版本保存会停止；先“另存副本”或导出，再重新读取云端版本。
4. 未同步内容按文件和编辑窗口保存在本机；重新打开时明确选择恢复。云端已有新版本时，只能把恢复内容另存副本，避免覆盖。浏览器存储不可用时请先导出；本机副本不替代云端备份。

首次连接会把原有流程图、画布云草稿各复制为一个命名文件，之后不会重复导入或覆盖。旧云草稿仍保留并独立使用；两者后续编辑互不跟随。工具临时页面的“存为新文件”可把当前浏览器草稿转成独立文件，本地原稿继续保留。

命名文件保存在 `tool_documents`；归档只改变状态，不删除内容。此工作台仍是站主单人文件库，没有多用户账号或实时协作。画布 JSON 可导出作为原始数据备份，本轮尚未新增画布文件导入与图形选择编辑。
