...
- 首页、文章分类与全文搜索、Markdown 阅读、关于与隐私说明。
- 内容页构建时预渲染为 HTML，生成页面标题、描述、canonical、Open Graph 和 sitemap。
...
文章目前为**明确标注的示例**，不代表站主的真实经历。工具草稿保存在当前浏览器，不包含云端同步、多人协作或后台文章编辑器。
...
Vite 开发模式适合编辑前端，不提供图床接口。测试完整 Pages/R2/D1 环境：
...
- `src/content/articles.ts`：新增文章对象，填写唯一 slug、标题、摘要、分类、日期和 Markdown 正文。正式文章将 `sample` 设为 `false`。
- `tokens.css`：全站设计变量。`src/styles.css`：布局和组件样式。
- `scripts/prerender.mjs`：当前 canonical / sitemap 域名为 `199819.xyz`；更换域名时同步修改这里。
...
- Pages 静态请求不进入本项目的 Functions；`public/_routes.json` 仅匹配 `/api/*` 与 `/images/*`。[Functions routing](https://developers.cloudflare.com/pages/functions/routing/)
